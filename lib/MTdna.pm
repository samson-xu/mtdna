...
java -jar $libDir/mitolib-0.1.2.jar haplochecker \\
...
$libDir/convert2annovar.pl --format vcf4 $outDir/$prefix.chrM.final.filter.vcf > $outDir/$prefix.chrM.final.filter.av
$libDir/table_annovar.pl --buildver hg19 --remove --protocol ensGene,mitomap20190903,mitimpact3 --operation g,f,f --nastring . $outDir/$prefix.chrM.final.filter.av $annotDir --outfile $outDir/$prefix.chrM.final
$libDir/add_AF.pl $outDir/$prefix.chrM.final.filter.vcf $outDir/$prefix.chrM.final.hg19_multianno.txt > $outDir/$prefix.chrM.final.result.xls
